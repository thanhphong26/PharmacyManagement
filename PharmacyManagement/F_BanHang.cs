...
namespace PharmacyManagement
{
    public partial class F_BanHang : Form
    {
        ...
        string maLo;
        int giaBanSp;
        public F_BanHang(string maCuaHang)
        {
            InitializeComponent();
            maCH = maCuaHang;      
            LoadCbbDanhMuc();
        ...
        private void LoadSp(DataTable data)
        {
            ...
            for (int i = 0; i < rowCount; i++)
            {
                ...
                //byte[] imageSp = ((byte[])data.Rows[i]["Image"]);
                F1SanPham f1SanPham = new F1SanPham(maSp, tenSp, chucNang);
        ...
        private void dgvLoSpOfCH_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                nUDSoluong.Maximum = int.Parse(dgvLoSpOfCH.Rows[e.RowIndex].Cells["SoLuong"].Value.ToString());
                nUDSoluong.Value = int.Parse(dgvLoSpOfCH.Rows[e.RowIndex].Cells["SoLuong"].Value.ToString());
                    
                maLo = dgvLoSpOfCH.Rows[e.RowIndex].Cells["MaLO"].Value.ToString();
            }
        }

        private void button1_Click(object sender, EventArgs e)
        {
            object[] rowData = {txtNameSp.Text,nUDSoluong.Value.ToString(), (nUDSoluong.Value*giaBanSp).ToString(), maLo};
        ...
        private void btnSearchKH_Click(object sender, EventArgs e)
        {
           DataRow dtRow  = khachHangDAO.SelectKH_By_SDT(txtSDT.Text);
            if (dtRow != null)
            {
                ...
                txtDiem.Text = dtRow["Diem"].ToString();
